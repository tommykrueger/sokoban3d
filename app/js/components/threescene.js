...
export default class ThreeScene {
  ...
  constructor (options = {}) {
    ...
    this.worldID = parseInt(this.utils.getParam('world')) || 1;
    this.levelID = parseInt(this.utils.getParam('level')) || 1;
  ...
  loadLevel (options = {}) {
    ...
    this.worldID = options.worldID || this.worldID;
    this.levelID = options.levelID || this.levelID;

		$.ajax({
		  url: './json/world-'+ this.worldID +'/level'+ this.levelID +'.json?time=' + Math.random(),
		  dataType: 'json',
		  success: (level) => {
        ...
        this.$interface.setLevelName(level);


        let dialog = new Dialog({
          app: this,
          title: 'Start Level',
          text: '<button class="button button-game" id="btn-start-game">Start Game</button>',
          actions: [
            {
              id: '#btn-start-game',
              action: 'start'
  ...
  loadNextLevel () {
    ...
    window.location.replace( window.location.pathname + '?world=' + this.worldID + '&level=' + (this.levelID + 1) );
